'use client';

import React, { useState, useMemo } from 'react';
import { TimelineDayId, DayForecastPoint } from '../../../../lib/types';
import { HISTORICAL_PATIENT_DATA, TIMELINE_KEYS, TIMELINE_FORECAST_MAP } from '../../../../lib/mockData';

interface ForecastChartProps {
  selectedDay: TimelineDayId;
  onSelectDay: (day: TimelineDayId) => void;
}

export const ForecastChart: React.FC<ForecastChartProps> = ({
  selectedDay,
  onSelectDay,
}) => {
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);

  // SVG dimensions
  const width = 1000;
  const height = 340;
  const padding = { top: 35, right: 40, bottom: 45, left: 60 };
  const innerWidth = width - padding.left - padding.right;
  const innerHeight = height - padding.top - padding.bottom;

  // Value boundaries: min patients 320, max patients 580, threshold at 500
  const minVal = 320;
  const maxVal = 580;
  const capacityThreshold = 500;

  // Scalers
  const getY = (val: number) => {
    const clamped = Math.max(minVal, Math.min(maxVal, val));
    return innerHeight - ((clamped - minVal) / (maxVal - minVal)) * innerHeight + padding.top;
  };

  // Historical data points (12 days)
  const histCount = HISTORICAL_PATIENT_DATA.length;
  // Forecast points (8 points: NOW to SUN)
  const forecastCount = TIMELINE_KEYS.length;
  const totalPoints = histCount + forecastCount - 1; // NOW overlaps with last historical

  const getX = (index: number) => {
    return padding.left + (index / totalPoints) * innerWidth;
  };

  // Coordinates for historical path
  const historicalCoords = useMemo(() => {
    return HISTORICAL_PATIENT_DATA.map((d, i) => ({
      x: getX(i),
      y: getY(d.admissions),
      admissions: d.admissions,
      date: d.date,
      occupancy: d.occupancyPercent,
    }));
  }, [histCount, innerWidth]);

  // Coordinates for forecast path
  const forecastCoords = useMemo(() => {
    return TIMELINE_KEYS.map((key, i) => {
      const point = TIMELINE_FORECAST_MAP[key];
      const globalIndex = histCount - 1 + i;
      return {
        key,
        x: getX(globalIndex),
        y: getY(point.totalAdmittedPatients),
        yLower: getY(point.confidenceInterval.lowerBound),
        yUpper: getY(point.confidenceInterval.upperBound),
        data: point,
      };
    });
  }, [histCount, innerWidth]);

  // SVG Paths
  const histPathD = useMemo(() => {
    return historicalCoords.reduce((acc, pt, i) => {
      return i === 0 ? `M ${pt.x} ${pt.y}` : `${acc} L ${pt.x} ${pt.y}`;
    }, '');
  }, [historicalCoords]);

  const forecastPathD = useMemo(() => {
    return forecastCoords.reduce((acc, pt, i) => {
      return i === 0 ? `M ${pt.x} ${pt.y}` : `${acc} L ${pt.x} ${pt.y}`;
    }, '');
  }, [forecastCoords]);

  // Confidence interval area polygon
  const confidenceBandD = useMemo(() => {
    if (forecastCoords.length === 0) return '';
    const topForward = forecastCoords.map((pt) => `${pt.x},${pt.yUpper}`).join(' ');
    const bottomBackward = [...forecastCoords].reverse().map((pt) => `${pt.x},${pt.yLower}`).join(' ');
    return `M ${forecastCoords[0].x},${forecastCoords[0].yUpper} ${topForward} L ${bottomBackward} Z`;
  }, [forecastCoords]);

  const thresholdY = getY(capacityThreshold);

  // Selected day X coordinate
  const selectedIndex = TIMELINE_KEYS.indexOf(selectedDay);
  const selectedX = getX(histCount - 1 + selectedIndex);

  // Breach point occurs at Friday (T+5)
  const breachPoint = forecastCoords.find((pt) => pt.key === 'FRI') || forecastCoords[5];

  // Active hover data calculation
  const activePoint = hoverIndex !== null && hoverIndex >= histCount - 1
    ? forecastCoords[hoverIndex - (histCount - 1)]
    : null;

  return (
    <div className="relative w-full overflow-hidden select-none">
      <svg
        viewBox={`0 0 ${width} ${height}`}
        className="w-full h-auto overflow-visible"
        onMouseLeave={() => setHoverIndex(null)}
      >
        <defs>
          {/* Gradient for Forecast Line */}
          <linearGradient id="forecastGradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#D3FD50" stopOpacity="0.8" />
            <stop offset="60%" stopColor="#F97316" stopOpacity="0.9" />
            <stop offset="100%" stopColor="#EF4444" stopOpacity="1" />
          </linearGradient>

          {/* Shaded confidence interval gradient */}
          <linearGradient id="confidenceFill" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#D3FD50" stopOpacity="0.16" />
            <stop offset="100%" stopColor="#D3FD50" stopOpacity="0.03" />
          </linearGradient>

          {/* Area fill under historical curve */}
          <linearGradient id="historicalFill" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="#ffffff" stopOpacity="0.08" />
            <stop offset="100%" stopColor="#ffffff" stopOpacity="0.0" />
          </linearGradient>
        </defs>

        {/* Horizontal Gridlines & Y-Axis Labels */}
        {[350, 400, 450, 500, 550].map((level) => {
          const yPos = getY(level);
          const isThreshold = level === 500;
          return (
            <g key={level}>
              <line
                x1={padding.left}
                y1={yPos}
                x2={width - padding.right}
                y2={yPos}
                stroke={isThreshold ? '#EF4444' : 'rgba(255, 255, 255, 0.07)'}
                strokeWidth={isThreshold ? 1.5 : 1}
                strokeDasharray={isThreshold ? '4 4' : '2 4'}
              />
              <text
                x={padding.left - 10}
                y={yPos + 3.5}
                textAnchor="end"
                className={`text-[10px] font-mono tabular-nums ${isThreshold ? 'fill-[#EF4444] font-bold' : 'fill-white/40'
                  }`}
              >
                {level}
              </text>
            </g>
          );
        })}

        {/* Capacity Threshold Label Bar */}
        <g>
          <rect
            x={width - padding.right - 215}
            y={thresholdY - 18}
            width="215"
            height="16"
            fill="#EF4444"
            fillOpacity="0.15"
            stroke="#EF4444"
            strokeWidth="0.8"
            rx="3"
          />
          <text
            x={width - padding.right - 10}
            y={thresholdY - 6}
            textAnchor="end"
            className="text-[9px] font-mono font-bold fill-[#EF4444] uppercase tracking-wider"
          >
            HOSPITAL CAPACITY LIMIT: 500 BEDS
          </text>
        </g>

        {/* Vertical Separator: Historic vs 7-Day Forecast */}
        <line
          x1={getX(histCount - 1)}
          y1={padding.top}
          x2={getX(histCount - 1)}
          y2={height - padding.bottom}
          stroke="#D3FD50"
          strokeWidth={1}
          strokeDasharray="3 3"
          strokeOpacity={0.4}
        />
        <text
          x={getX(histCount - 1) - 8}
          y={padding.top - 8}
          textAnchor="end"
          className="text-[9px] font-mono uppercase fill-white/40 tracking-wider"
        >
          ← HISTORICAL ACTUAL
        </text>
        <text
          x={getX(histCount - 1) + 8}
          y={padding.top - 8}
          textAnchor="start"
          className="text-[9px] font-mono uppercase fill-[#D3FD50] font-semibold tracking-wider"
        >
          7-DAY ML FORECAST →
        </text>

        {/* Confidence Band Polygon */}
        <path d={confidenceBandD} fill="url(#confidenceFill)" />

        {/* Confidence Bound Outline Dashes */}
        {forecastCoords.map((pt, i) => {
          if (i === 0) return null;
          const prev = forecastCoords[i - 1];
          return (
            <g key={`bound-${i}`}>
              <line
                x1={prev.x}
                y1={prev.yUpper}
                x2={pt.x}
                y2={pt.yUpper}
                stroke="#D3FD50"
                strokeWidth={0.8}
                strokeDasharray="2 2"
                strokeOpacity={0.4}
              />
              <line
                x1={prev.x}
                y1={prev.yLower}
                x2={pt.x}
                y2={pt.yLower}
                stroke="#D3FD50"
                strokeWidth={0.8}
                strokeDasharray="2 2"
                strokeOpacity={0.4}
              />
            </g>
          );
        })}

        {/* Historical Actual Line */}
        <path
          d={histPathD}
          fill="none"
          stroke="#9CA3AF"
          strokeWidth={2}
          strokeLinecap="round"
          strokeLinejoin="round"
        />

        {/* Historical Data Dots */}
        {historicalCoords.map((pt, i) => (
          <circle
            key={`hist-${i}`}
            cx={pt.x}
            cy={pt.y}
            r={2.5}
            className="fill-[#9CA3AF]"
          />
        ))}

        {/* Forecast Line */}
        <path
          d={forecastPathD}
          fill="none"
          stroke="url(#forecastGradient)"
          strokeWidth={3}
          strokeLinecap="round"
          strokeLinejoin="round"
        />

        {/* Forecast Data Dots & Hit Targets */}
        {forecastCoords.map((pt, i) => {
          const isCurrentSelected = pt.key === selectedDay;
          const isBreached = pt.data.isBreach;
          return (
            <g
              key={`fc-${i}`}
              className="cursor-pointer"
              onClick={() => onSelectDay(pt.key)}
              onMouseEnter={() => setHoverIndex(histCount - 1 + i)}
            >
              {/* Invisible large hit area */}
              <circle cx={pt.x} cy={pt.y} r={18} fill="transparent" />

              {/* Point Halo */}
              {isCurrentSelected && (
                <circle
                  cx={pt.x}
                  cy={pt.y}
                  r={8}
                  fill="none"
                  stroke={isBreached ? '#EF4444' : '#D3FD50'}
                  strokeWidth={1.5}
                  className="animate-pulse"
                />
              )}

              <circle
                cx={pt.x}
                cy={pt.y}
                r={isCurrentSelected ? 4.5 : 3.5}
                fill={isBreached ? '#EF4444' : '#D3FD50'}
                stroke="#080909"
                strokeWidth={2}
              />
            </g>
          );
        })}

        {/* SELECTED DAY VERTICAL GUIDE */}
        <line
          x1={selectedX}
          y1={padding.top - 5}
          x2={selectedX}
          y2={height - padding.bottom}
          stroke="#D3FD50"
          strokeWidth={1.5}
          strokeDasharray="3 2"
          strokeOpacity={0.8}
        />

        {/* CAPACITY BREACH CALLOUT MARKER ON FRIDAY */}
        {breachPoint && (
          <g className="cursor-pointer" onClick={() => onSelectDay('FRI')}>
            {/* Pulsing Beacon Halo */}
            <circle
              cx={breachPoint.x}
              cy={thresholdY}
              r={14}
              fill="#EF4444"
              fillOpacity={0.2}
              className="animate-ping"
            />
            <circle
              cx={breachPoint.x}
              cy={thresholdY}
              r={5}
              fill="#EF4444"
              stroke="#ffffff"
              strokeWidth={1.5}
            />

            {/* Vertical connector line */}
            <line
              x1={breachPoint.x}
              y1={thresholdY}
              x2={breachPoint.x}
              y2={thresholdY - 45}
              stroke="#EF4444"
              strokeWidth={1.5}
            />

            {/* Badge Card */}
            <g transform={`translate(${breachPoint.x - 70}, ${thresholdY - 72})`}>
              <rect
                x="0"
                y="0"
                width="140"
                height="32"
                rx="6"
                fill="#15181b"
                stroke="#EF4444"
                strokeWidth="1.2"
                filter="drop-shadow(0 4px 12px rgba(239, 68, 68, 0.35))"
              />
              <text
                x="70"
                y="13"
                textAnchor="middle"
                className="text-[9px] font-mono font-black fill-[#EF4444] uppercase tracking-wider"
              >
                CAPACITY BREACH
              </text>
              <text
                x="70"
                y="24"
                textAnchor="middle"
                className="text-[8px] font-mono font-bold fill-white uppercase"
              >
                4 DAYS • +9 ICU SHORTAGE
              </text>
            </g>
          </g>
        )}

        {/* X-AXIS LABELS */}
        {/* Historical X Labels (sparse) */}
        {historicalCoords.map((pt, i) => {
          if (i % 3 !== 0) return null;
          return (
            <text
              key={`x-hist-${i}`}
              x={pt.x}
              y={height - padding.bottom + 18}
              textAnchor="middle"
              className="text-[9px] font-mono fill-white/30"
            >
              {pt.date}
            </text>
          );
        })}

        {/* 7-Day Forecast X Labels */}
        {forecastCoords.map((pt) => {
          const isSelected = pt.key === selectedDay;
          const isBreachDay = pt.data.isBreach;
          return (
            <g
              key={`x-fc-${pt.key}`}
              className="cursor-pointer"
              onClick={() => onSelectDay(pt.key)}
            >
              <text
                x={pt.x}
                y={height - padding.bottom + 18}
                textAnchor="middle"
                className={`text-[10px] font-mono font-bold uppercase tracking-wider ${isSelected
                    ? isBreachDay
                      ? 'fill-[#EF4444]'
                      : 'fill-[#D3FD50]'
                    : isBreachDay
                      ? 'fill-[#EF4444]/70'
                      : 'fill-white/60'
                  }`}
              >
                {pt.key}
              </text>
              <text
                x={pt.x}
                y={height - padding.bottom + 30}
                textAnchor="middle"
                className="text-[8px] font-mono fill-white/30 uppercase"
              >
                {pt.key === 'NOW' ? 'Today' : `T+${pt.data.dayOffset}`}
              </text>
            </g>
          );
        })}
      </svg>

      {/* Interactive Tooltip Card */}
      {activePoint && (
        <div
          className="absolute pointer-events-none z-30 p-2.5 rounded-lg bg-[#121417] border border-white/20 shadow-xl text-xs font-mono"
          style={{
            left: `${Math.min(85, Math.max(10, (activePoint.x / width) * 100))}%`,
            top: '12%',
            transform: 'translateX(-50%)',
          }}
        >
          <div className="flex items-center justify-between gap-3 pb-1 border-b border-white/[0.08] mb-1.5">
            <span className="text-white font-bold">{activePoint.data.label}</span>
            <span
              className={`px-1.5 py-0.2 text-[9px] font-bold rounded ${activePoint.data.isBreach
                  ? 'bg-[#EF4444]/20 text-[#EF4444]'
                  : 'bg-[#D3FD50]/20 text-[#D3FD50]'
                }`}
            >
              {activePoint.data.isBreach ? 'BREACH PREDICTED' : 'WITHIN TOLERANCE'}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
            <div className="text-white/50">Admitted Patients:</div>
            <div className="text-right text-white font-bold tabular-nums">
              {activePoint.data.totalAdmittedPatients} / 500
            </div>
            <div className="text-white/50">95% Confidence:</div>
            <div className="text-right text-white/80 tabular-nums">
              [{activePoint.data.confidenceInterval.lowerBound} – {activePoint.data.confidenceInterval.upperBound}]
            </div>
            <div className="text-white/50">ICU Projection:</div>
            <div
              className={`text-right font-bold tabular-nums ${activePoint.data.resources.icu.shortageDelta > 0
                  ? 'text-[#EF4444]'
                  : 'text-white'
                }`}
            >
              {activePoint.data.resources.icu.projectedDemand} / 50 Beds
            </div>
          </div>
        </div>
      )}
    </div>
  );
};
