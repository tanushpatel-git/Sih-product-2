...
export default function PatientHistoryPage() {
  ...
  return <main className="min-h-screen bg-[#f4f6f5] px-5 py-8 text-[#17221f] sm:px-10">
    <div className="mx-auto max-w-4xl">
      ...
      <section className="mt-7 rounded-[22px] border border-[#dfe8e3] bg-white p-6 shadow-[0_8px_30px_rgba(20,30,25,.025)]"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#edf4f1] text-[#52786d]"><FileUp size={17}/></span><div><h2 className="text-sm font-semibold">Upload a prescription</h2><p className="text-xs text-[#71807a]">Your uploaded prescription is added to this history and visible to your doctor.</p></div></div><form onSubmit={submit} className="mt-5 grid gap-3 md:grid-cols-2"><input type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onChange={(e) => setFile(e.target.files?.[0] || null)} required className="text-xs"/><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Prescription title" required className="h-11 rounded-xl border border-[#dfe5e2] bg-[#f9faf9] px-3 text-sm outline-none focus:border-[#aab5b0]"/><textarea value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="Optional note" rows={2} className="rounded-xl border border-[#dfe5e2] bg-[#f9faf9] p-3 text-sm outline-none focus:border-[#aab5b0] md:col-span-2"/><div className="flex items-center gap-3 md:col-span-2"><button disabled={uploading} className="flex h-11 items-center gap-2 rounded-xl bg-[#17221f] px-4 text-sm font-medium text-white disabled:opacity-60">{uploading ? <LoaderCircle size={15} className="animate-spin"/> : <Upload size={15}/>}{uploading ? "Uploading…" : "Upload prescription"}</button>{message && <p className="text-xs text-emerald-700">{message}</p>}{error && <p className="text-xs text-red-600">{error}</p>}</div></form></section>
