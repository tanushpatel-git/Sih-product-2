...
import {
  ...
  buildStrokeFeatures,
  buildAnemiaFeatures,
  buildBreastCancerFeatures,
  buildDiabetesFeatures,
  buildHeartDiseaseFeatures,
  buildHeartFailureFeatures,
  buildKidneyFeatures,
  buildLiverFeatures,
  riskBandFromProbability,
  heartFailureRiskFromProbability,
  diabetesRiskFromProbability,
  breastRiskFromProbability,
  kidneyRiskFromStage,
  roundProb,
} from "./server-predict";
...
export default function ClinicalDashboardPage() {
  ...
  // ── Server-backed predictions (ACTUAL trained .pkl models via services/ml) ──
  // Local inference engines below remain the source for factor attributions and
  // clinical narratives; every displayed probability/class/status is overridden
  // with the value produced by the real model artifact.
  const {
    prediction: strokePred,
  } = useModelPrediction("stroke", activeModelKey === "stroke", () => buildStrokeFeatures(strokeInputs));
  const {
    prediction: heartDiseasePred,
  } = useModelPrediction("heartDisease", activeModelKey === "heartDisease", () => buildHeartDiseaseFeatures(hdInputs));
  const {
    prediction: anemiaPred,
  } = useModelPrediction("anemia", activeModelKey === "anemia", () => buildAnemiaFeatures(anemiaInputs));
  const {
    prediction: breastCancerPred,
  } = useModelPrediction("breastCancer", activeModelKey === "breastCancer", () => buildBreastCancerFeatures(bcInputs));
  const {
    prediction: diabetesPred,
  } = useModelPrediction("diabetes", activeModelKey === "diabetes", () => buildDiabetesFeatures(diabetesInputs));
  const {
    prediction: heartFailurePred,
  } = useModelPrediction("heartFailure", activeModelKey === "heartFailure", () => buildHeartFailureFeatures(hfInputs));
  const {
    prediction: kidneyPred,
  } = useModelPrediction("kidneyDisease", activeModelKey === "kidneyDisease", () => buildKidneyFeatures(kidneyInputs));
  const {
    prediction: liverPred,
  } = useModelPrediction("liverDisease", activeModelKey === "liverDisease", () => buildLiverFeatures(liverInputs));

  // ── Inference engines (client-side) ──────────────────────────────────────────
  const liveStrokeResult = useMemo(() => {
    const base = runStrokeInference(strokeInputs);
    if (!strokePred) return base;
    const cls = strokePred.predicted_class === 1 || strokePred.class_index === 1;
    const prob = roundProb(strokePred.probability_percent);
    return {
      ...base,
      predictionClass: (cls ? 1 : 0) as 0 | 1,
      probabilityPercent: prob,
      logit: strokePred.raw_score ?? base.logit,
      statusLabel: cls ? "HIGH STROKE RISK DETECTED" : "LOW RISK OF STROKE",
      riskBand: riskBandFromProbability(prob),
    };
  }, [strokeInputs, strokePred]);
  const liveHdResult = useMemo(() => {
    const base = runHeartDiseaseInference(hdInputs);
    if (!heartDiseasePred) return base;
    const cls = heartDiseasePred.predicted_class === 1 || heartDiseasePred.class_index === 1;
    const prob = roundProb(heartDiseasePred.probability_percent);
    return {
      ...base,
      predictionClass: (cls ? 1 : 0) as 0 | 1,
      probabilityPercent: prob,
      logit: heartDiseasePred.raw_score ?? base.logit,
      statusLabel: cls ? "CORONARY HEART DISEASE DETECTED" : "LOW CORONARY RISK",
      riskBand: riskBandFromProbability(prob),
    };
  }, [hdInputs, heartDiseasePred]);
  const liveAnemiaResult = useMemo(() => {
    const base = runAnemiaInference(anemiaInputs);
    if (!anemiaPred) return base;
    const cls = anemiaPred.predicted_class === 1 || anemiaPred.class_index === 1;
    const prob = roundProb(anemiaPred.probability_percent);
    let severity: "OPTIMAL" | "MILD DEFICIT" | "MODERATE" | "SEVERE" = "OPTIMAL";
    if (cls) {
      if (anemiaInputs.hemoglobin < 8.0) severity = "SEVERE";
      else if (anemiaInputs.hemoglobin < 10.5) severity = "MODERATE";
    ...
    return {
      ...
      statusLabel: cls ? "ANEMIA DETECTED" : "NO ANEMIA DETECTED",
      logit: anemiaPred.raw_score ?? base.logit,
    };
  }, [anemiaInputs, anemiaPred]);
  const liveBcResult = useMemo(() => {
    const base = runBreastCancerInference(bcInputs);
    if (!breastCancerPred) return base;
    const cls = breastCancerPred.predicted_class === 1 || breastCancerPred.class_index === 1;
    const prob = roundProb(breastCancerPred.probability_percent);
    return {
      ...base,
      predictionClass: (cls ? 1 : 0) as 0 | 1,
      probabilityPercent: prob,
      logit: breastCancerPred.raw_score ?? base.logit,
      statusLabel: cls ? "MALIGNANT" : "BENIGN",
      riskClassification: breastRiskFromProbability(
        cls ? 1 : 0,
        prob,
        bcInputs.area_worst,
        bcInputs.concave_points_worst
      ),
    };
  }, [bcInputs, breastCancerPred]);
  const liveDiabetesResult = useMemo(() => {
    const base = runDiabetesInference(diabetesInputs);
    if (!diabetesPred) return base;
    const cls = diabetesPred.predicted_class === 1 || diabetesPred.class_index === 1;
    const prob = roundProb(diabetesPred.probability_percent);
    return {
      ...base,
      predictionClass: (cls ? 1 : 0) as 0 | 1,
      probabilityPercent: prob,
      margin: diabetesPred.raw_score ?? base.margin,
      statusLabel: cls ? "DIABETES DETECTED" : "NO DIABETES DETECTED",
      riskClassification: diabetesRiskFromProbability(prob),
    };
  }, [diabetesInputs, diabetesPred]);
  const liveHfResult = useMemo(() => {
    const base = predictHeartFailure(hfInputs);
    if (!heartFailurePred) return base;
    const cls = heartFailurePred.predicted_class === 1 || heartFailurePred.class_index === 1;
    const prob = roundProb(heartFailurePred.probability_percent);
    const { riskClassification, hemodynamicStatus } = heartFailureRiskFromProbability(prob);
    return {
      ...base,
      predictionClass: (cls ? 1 : 0) as 0 | 1,
      probabilityPercent: prob,
      statusLabel: cls ? "HIGH RISK OF HEART FAILURE EVENT" : "COMPENSATED / LOW RISK OF EVENT",
      riskClassification,
      hemodynamicStatus,
    };
  }, [hfInputs, heartFailurePred]);
  const liveKidneyResult = useMemo(() => {
    const base = runKidneyDiseaseInference(kidneyInputs);
    if (!kidneyPred) return base;
    const idx = kidneyPred.class_index as 0 | 1 | 2 | 3 | 4;
    return {
      ...base,
      predictionClass: KIDNEY_LABELS[idx],
      stageIndex: idx,
      ckdRiskLevel: kidneyRiskFromStage(idx),
      probabilityPercent: roundProb(kidneyPred.probability_percent),
    };
  }, [kidneyInputs, kidneyPred]);
  const liveLiverResult = useMemo(() => {
    const base = runLiverDiseaseInference(liverInputs);
    if (!liverPred) return base;
    const cls = liverPred.predicted_class === 1 || liverPred.class_index === 1;
    const prob = roundProb(liverPred.probability_percent);
    return {
      ...base,
      predictionClass: (cls ? 1 : 0) as 0 | 1,
      probabilityPercent: prob,
      statusLabel: cls ? "LIVER DISEASE DETECTED" : "HEALTHY HEPATIC FUNCTION",
      riskBand: riskBandFromProbability(prob),
    };
  }, [liverInputs, liverPred]);
