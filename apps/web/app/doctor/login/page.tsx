...
export function DoctorLoginPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (remember) {
        localStorage.setItem("vitaweave_remember_email", email.trim());
      } else {
        localStorage.removeItem("vitaweave_remember_email");
      }
    } catch (err: unknown) {
          ...
          <div className="w-full max-w-[470px]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <motion.button
                ...
                type="submit"
                disabled={loading}
                className="group flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-[#17201d] text-sm font-medium text-white shadow-[0_14px_35px_rgba(23,32,29,0.16)] transition hover:bg-[#25312d] disabled:opacity-50 disabled:pointer-events-none"
              >
                {loading ? "Signing in..." : "Enter clinical workspace"}
